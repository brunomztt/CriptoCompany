...
import { View, Text, FlatList, Image, TouchableOpacity, TextInput, ActivityIndicator, Alert, RefreshControl, BackHandler, Platform } from 'react-native';
...
const HomeScreen = ({ navigation }: { navigation: any }) => {
  ...
  `;
  const SearchSvg = `
      <svg xmlns="http://www.w3.org/2000/svg" width="17" height="17" fill="none" viewBox="0 0 24 30">
        <path stroke="#333" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 19a8 8 0 1 0 0-16 8 8 0 0 0 0 16Zm9-2-2.8-2.8"/>
      </svg>
  `;
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.containerbookmarks}>
        ...
        <TouchableOpacity style={styles.viewAllButton} onPress={() => navigation.navigate('CryptoScreen')}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={['#23EBC3']} />}
        />
      ) : (
        <View style={styles.noFavorites}>
          <Text style={styles.noFavoritesText}>Please add a cryptocurrency to your bookmarks.</Text>
        </View>
      )}
      <TouchableOpacity style={defaultstyles.button} onPress={() => navigation.navigate('ExchangeScreen')}>
        <Text style={defaultstyles.buttonText}>
          <SvgXml xml={ArrowTransferSvg} width="31" height="31" />
        </Text>
      </TouchableOpacity>
    </View>
  );
